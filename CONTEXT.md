...
that turns a drawing into 3D geometry (`Knot` → `Diagram` → `Knot3D` → `SecondaryLoop`).
...
the **Secondary** tab (read-only for the knots/crossings themselves; shows the same knots and
crossings as a static base, with the current **Secondary loop** drawn on top). Tab activation
is the single source of truth for whether the Secondary loop is shown — there's no separate
visibility toggle; switching to the Secondary tab shows the loop in both the Viewer and the
Secondary board, switching away hides it in both.
_Avoid_: view, panel (these are UI mechanics, not the domain concept — the concept is *which
board is showing and what it's for*)

**Secondary walk**:
The graph-walk that produces a `SecondaryLoop` (see [AGENTS.md](./AGENTS.md)) — starting at a
...
knot's own points on the **Secondary board** (heading forward from there, per **Going forward
...
The choice of which way the **Secondary walk** continues from a **crossing point**. Every
...
fourth would exactly retrace it. Shown as a clickable arrow on the **Secondary board**, next to
...
the knots — and therefore the crossings — can't change while the Secondary tab is showing.

**Arrival direction**:
The role and forward/backward step describing which line the **Secondary walk** was actually
...
**Secondary loop**:
The `SecondaryLoop` a secondary walk produces — an ordered sequence of points that alternates
between points inherited from a knot's diagram and **crossing points**. Shown in the Viewer
(3D) and, as an overlay on top of the knots' own lines, in the Secondary board (2D) — the same
...
if it closes into a smaller loop that doesn't reach back to the start. The Secondary board
surfaces `isClosed === false` to the user directly, since an unclosed loop can't produce a valid
Secondary surface.
_Avoid_: secondary, loop (on their own, ambiguous with the walk or with a knot's own polyline)

**Crossing point**:
A point in a secondary loop with no origin in any knot's 2D diagram — its coordinates are
...
knot's diagram and keeps that diagram point's identity. For the Secondary board's 2D overlay,
...
**Zigzag**:
A short run of **Secondary loop** points produced wherever a knot's own bridging triangle (built by
...
expected loop geometry, not a walk defect - only the **Secondary cap**, **Secondary wall**, and
...
**Secondary surface**:
The surface built from a Secondary loop's own points, shown only in the Viewer (there is no 2D
equivalent on the Secondary board). Rendered by `SecondarySurface.vue`, separate from the loop's
own line/point rendering, and hidden/shown by its own toggle (only offered while the Secondary
tab is active, alongside the loop it's built from). Made of two pieces, both driven by the same
`color`/`visible` props: the **Secondary cap** and the **Secondary wall**.
_Avoid_: secondary surface used loosely for the cap alone — the cap is one piece of this, not a
synonym for it

**Secondary cap**:
A flat, triangulated piece of the **Secondary surface**, built by flattening every one of a
Secondary loop's own points to its x/z position (the same projection a crossing point already
...
**Secondary wall**). Rendered by its own component (`SecondaryCap.vue`), used by
`SecondarySurface.vue`.
_Avoid_: cap surface (ambiguous with the loop or a knot's own surface)

**Secondary wall**:
The piece of the **Secondary surface** connecting the **Secondary cap** down to the loop's own
points at their real height. For every pair of adjacent Secondary loop points (wrapping the last
...
Rendered by its own component (`SecondaryWall.vue`), used by `SecondarySurface.vue`.
_Avoid_: wall surface (ambiguous with the cap or a knot's own surface)

**Secondary intersection**:
A point where the **Secondary wall**'s triangles cross a knot's own 3D surface. The cap is
...
**Cap shift**:
A live offset (`capShiftDistance`) on the **Secondary cap**'s boundary, coupled to the
**Secondary wall**'s top edge — the wall's bottom edge and the knots' own points are untouched.
