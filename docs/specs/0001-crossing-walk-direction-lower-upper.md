...
(`src/data/secondary-walk.ts`) and is reset to defaults every time the Secondary tab mounts
(`SecondaryBoard.vue:76-78`), per CONTEXT.md - there's no persisted state across sessions that
...
Add to `secondaries.ts`, near `getCrossingId`:

```ts
function findRealIntersection(surfaceIntersection: SecondariesPoint["surfaceIntersection"]) {
...
function getCrossingRole(point: SecondariesPoint): "lower" | "upper" | undefined {
...
(`secondaries.ts:474-479`) - consider having `isAtRealIntersection` call `findRealIntersection`
...
## `advance` (`secondaries.ts:287-323`)
...
## `getDefaultCrossingWalkDirections` (`secondaries.ts:228-240`)
...
## `getWalkStart` (`secondaries.ts:361-395`)
...
## `getCrossingWalkArrows` (`secondaries.ts:488-538`)
...
- The arrival-exclusion logic (`secondaries.ts:512-519`) currently always excludes a `"self*"`
...
## `getDirectionTarget` (`secondaries.ts:444-459`)
...
## UI (`SecondaryBoard.vue`)

No structural change expected: `cycleDirection` only walks `arrow.options` as an opaque array
(`SecondaryBoard.vue:156-164`) and doesn't inspect direction values itself - it inherits the new
...
- `AGENTS.md` doesn't currently document `secondaries.ts` internals at all (deliberately, per its
