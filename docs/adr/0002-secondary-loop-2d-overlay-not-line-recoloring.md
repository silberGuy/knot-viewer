# Secondary loop drawn as its own 2D overlay, not by recoloring existing lines

To show the Secondary loop on the Secondary board (not just the Viewer), we needed a way to
...
We rejected it: a Secondary loop's points alternate between ordinary points (inherited from a
...
The Secondary board computes this projected loop itself, and draws it as one continuous overlay
...
`drawingData` — see below) — the same data the Viewer's `knots3D`/`secondaryLoop` are computed
...
`secondaries.ts`) that passed at one scale failed at the other, so the Viewer and the board's
overlay computed visibly different loops. Both boards now sharing the same raw data removes that
class of bug entirely, and also means the Secondary board's knot lines line up with the Drawing
...
never touches `knots3D` or `secondaryLoop` — it's applied after that geometry is already
computed, purely to keep the existing camera framing working. The Secondary board's 2D overlay,
