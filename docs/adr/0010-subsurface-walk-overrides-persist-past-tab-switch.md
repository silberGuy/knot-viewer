# Subsurface walk overrides persist across tabs and Save, pruned instead of reset

Selected start point and crossing-walk-direction overrides used to reset every time the
Subsurface tab was (re-)entered, on the reasoning that the knots can't change while it's showing
...
(`subsurfaceWalkStore`) rather than folded into `DrawingData` - they're save-file content, but not
part of what defines the drawing itself. Since point ids are positional (`${knotId}-${index}`),
a drawing edit can still invalidate a reference; on re-entering the Subsurface tab we prune only
